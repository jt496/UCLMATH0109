...
/-
  ...
  # Assessed Sheet 3
   Covering material from sheet3A,B,C
 `norm_num` `linarith` `ring` `apply?` `exact?`
  `refine` `convert` `congr` `have`
-/
...
by
  ...
  · ring
  rw [hk,this, Nat.add_sub_self_left (m^2)]
...
by
  ...
  · have hbpos : 0 < b
    ...
    use N; intro n hn
    have : |x n * b - a * b| = |x n - a| * b
    · have : |x n * b  - a * b| = |x n - a| * |b|
      · have : |x n * b - a * b| = |(x n - a)* b|
        · congr! 1
          exact Eq.symm (sub_mul (x n) a b) -- found via apply?
        rw [this]
        exact abs_mul (x n - a) b -- found via apply?
      rw [this]
      congr!
      exact abs_of_pos hbpos -- found via apply?
    rw [this]
    specialize hN n hn
    exact Iff.mp (lt_div_iff hbpos) hN -- found via apply?
