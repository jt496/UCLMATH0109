...
If `x : Option A` then we can again think of the as an `or` statement
...
is `V ⊕ (Option V)`. We can think of this as the new vertex set consists
...
/- An example of the kind of definitional you may want to start with.-/
@[simp]
...
set_option maxRecDepth 200000 -- This will help if your computer is slow
