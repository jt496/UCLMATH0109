...
Given a real number `x`, define a sequence `seq x` and prove the the
convergents of the continued fraction of `seq x` converge to `x`.

Calculate the sequence `seq x` in the case `x = Real.sqrt d` for some positive integer `d`.
-/

noncomputable def floor : ℝ → ℤ := fun x ↦ x.exists_floor.choose

noncomputable def fracPart (x : ℝ) := x - floor x

noncomputable def remainder (x : ℝ) : ℕ → ℝ
| 0     => x
| n+1   => 1 / fracPart (remainder x n)

noncomputable def seq (x : ℝ) (n : ℕ) := floor (remainder x n)
