...
/-
Define the field embeddings as ring homomorphisms.
-/
def σ₁ : R →+* ℝ := sorry
def σ₂ : R →+* ℂ := sorry
def σ₃ : R →+* ℂ := sorry
...
noncomputable def ℓ : Rˣ →* ℝ where
  toFun u := Real.log |σ₁ u.val|
  map_one' := sorry
  map_mul' := sorry

lemma step1 : ℓ fund > 1 := by sorry
