...
-- Facts about metric spaces and sequences
#check dist_self
#check dist_comm
#check dist_triangle
...
-- When working with ℕ and Ndist in part 2 the following lemmas may be useful
-- Also the tactic `split_ifs` for dealing with `ite` definitions e.g. Ndist
#check cast_nonneg
#check div_nonneg
#check div_le_one
#check cast_one
...
-- Throughout the tactics `simp?`, `norm_cast`, `norm_num` and `linarith` will be useful.
/-
