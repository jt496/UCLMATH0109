...
-- Facts about metric spaces that may be useful
#check dist_self
#check dist_comm
#check dist_triangle
#check isOpen_iff
...
-- Facts about sets and Finsets that may be useful
#check mem_iInter
#check Finset.mem_range
#check Finset.mem_image
#check Finset.min'_mem
#check Finset.lt_min'_iff
...
/-
The following will be useful when defining the function `func U` below.
...
lemma first (hnin : x ∉ ⋂ n, U n) : ∃ N, x ∉ U N ∧ ∀ n, n < N → x ∈ U n :=
by
  sorry
...
-- the tactics `split_ifs`, `dsimp`, `contradiction` and `rfl` will be useful here

/-- If ∀ n, is_open (U n),  then func U is cts at every x ∈ ⋂ n, U n -/
theorem iInter_ss_SetOfContinuousAt (hopen : ∀ n, IsOpen (U n)) : (⋂ n, U n) ⊆ SetOfContinuousAt (func U) :=by
  sorry

/-- The SetOfContinuousAt is a subset of ⋂ n, U n-/
theorem setOfContinuousAt_ss_iInter : SetOfContinuousAt (func U) ⊆ ⋂ n, U n :=by
  sorry
...
/-!
Possible extension: give an example of a metric space (X,d) and a sequence of open sets in
 X such that no function f : X → ℝ has G as its set of continuity.
-/
