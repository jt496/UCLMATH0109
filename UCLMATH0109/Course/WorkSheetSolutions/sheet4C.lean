...
by
  ...
  · intro k
    induction k with
    | zero =>
      rw [Nat.zero_eq,pow_zero,one_mul,mul_one]
    | succ n ih =>
      rw [Nat.succ_eq_add_one,pow_add,←mul_assoc,ih,pow_one,mul_assoc,h,←mul_assoc,←pow_add,
        two_mul,two_mul,← add_assoc,Nat.succ_add]
  induction n with
  | zero =>
    rw [Nat.zero_eq,pow_zero,one_mul,pow_zero,mul_one,pow_one]
  | succ n ih =>
    rw [Nat.succ_eq_add_one,add_comm,pow_add,mul_assoc,ih,pow_one,← mul_assoc,this,mul_assoc]
    congr
    rw [pow_add,pow_one]
