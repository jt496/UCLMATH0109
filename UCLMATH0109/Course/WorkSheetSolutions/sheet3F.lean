...
by
  ...
  · trans 4 * b
    · use 2
      ring
    · exact Iff.mp Int.dvd_neg h2 -- used `exact?` here
/-
...
(a / b : ℕ) * (b / c : ℕ) = (a : ℝ) / c :=
by
  have : c ∣ a  --following hint here.
  · trans b
    · assumption
    · assumption
  norm_cast     --following hint here.
  rw [Nat.div_mul_div_comm h1 h2, mul_comm, Nat.mul_div_mul] -- using lemmas mentioned above.
  exact Nat.pos_of_ne_zero h -- using `exact?`.
...
by
  rw [div_lt_one,Nat.choose_succ_succ]
  · norm_cast
    refine Nat.lt_add_of_pos_left ?h -- found using `apply?`
    exact Nat.choose_pos h           -- found using `exact?`
  · norm_cast
    rw [Nat.choose_succ_succ]
    apply Nat.add_pos_left -- guessed.
    exact Nat.choose_pos h -- found using `exact?`
