...
by
  simp only [Commute.refl, Commute.pow_pow, h, Commute.pow_right, Commute.mul_left]

-- 04
example [Group G] [Group H] (x y : G) (φ : G →* H) (h : φ x = φ y) : φ (y⁻¹*x) =1 :=
by
  simp only [map_mul, map_inv, h, mul_left_inv]
...
by
  simp only [Int.coe_natAbs, abs_eq_self, sub_nonneg, h]
...
by
  simp only [Int.emod_add_emod, h, Int.add_emod_emod]
...
by
  ...
  · right; exact hbc
  · left;
    simp only [Finset.mem_singleton, hbc, Finset.mem_insert, hab, hac, or_self, not_false_eq_true,
